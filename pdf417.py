...
class PDF417(object):
    ...
    TEXTSUBMODES = [
        [0x41, 0x42, 0x43, 0x44, 0x45, 0x46, 0x47, 0x48, 0x49, 0x4a, 0x4b, 0x4c, 0x4d, 0x4e, 0x4f, 0x50, 0x51, 0x52,
         0x53, 0x54, 0x55, 0x56, 0x57, 0x58, 0x59, 0x5a, 0x20, 0xFD, 0xFE, 0xFF],

        [0x61, 0x62, 0x63, 0x64, 0x65, 0x66, 0x67, 0x68, 0x69, 0x6a, 0x6b, 0x6c, 0x6d, 0x6e, 0x6f, 0x70, 0x71, 0x72,
         0x73, 0x74, 0x75, 0x76, 0x77, 0x78, 0x79, 0x7a, 0x20, 0xFD, 0xFE, 0xFF],

        [0x30, 0x31, 0x32, 0x33, 0x34, 0x35, 0x36, 0x37, 0x38, 0x39, 0x26, 0x0d, 0x09, 0x2c, 0x3a, 0x23, 0x2d, 0x2e,
         0x24, 0x2f, 0x2b, 0x25, 0x2a, 0x3d, 0x5e, 0xFB, 0x20, 0xFD, 0xFE, 0xFF],

        [0x3b, 0x3c, 0x3e, 0x40, 0x5b, 0x5c, 0x5d, 0x5f, 0x60, 0x7e, 0x21, 0x0d, 0x09, 0x2c, 0x3a, 0x0a, 0x2d, 0x2e,
         0x24, 0x2f, 0x22, 0x7c, 0x2a, 0x28, 0x29, 0x3f, 0x7b, 0x7d, 0x27, 0xFF],
    ]
    ...
    def __init__(self, code, ecl=-1, aspectratio=2):
        sequence = self._get_sequence(code)
        code_words = []

        self.barcode_array = {}

        for i in range(len(sequence)):
            cw = self._get_compaction(sequence[i][0], sequence[i][1])
            code_words = code_words + cw

        if code_words[0] == 900:
            code_words.pop(0)

        numcw = len(code_words)

        if numcw > 925:
            return

        ecl = self._get_error_correction_level(ecl, numcw)
        err_size = 2 << ecl

        nce = numcw + err_size + 1

        cols = round((math.sqrt(4761 + (68 * 2 * self.ROWHEIGHT * nce)) - 69) / 34)
        ...
        if pad > 0:
            if (size - rows) == nce:
                ...
                code_words = code_words + [900] * int(pad)

        sld = size - err_size
        code_words.insert(0, sld)
        ecw = self._get_error_correction(code_words, ecl)

        code_words += ecw

        pstart = '0' * self.QUIETH + self.START_PATTERN
        pstop = self.STOP_PATTERN + '0' * self.QUIETH

        self.barcode_array['num_rows'] = (rows * self.ROWHEIGHT) + (2 * self.QUIETV)
        self.barcode_array['num_cols'] = ((cols + 2) * 17) + 35 + (2 * self.QUIETH)
        self.barcode_array['bcode'] = []

        if self.QUIETV > 0:
            empty_row = [0] * int(self.barcode_array['num_cols'])

            for i in range(0, self.QUIETV):
                self.barcode_array['bcode'].append(empty_row)
        ...
        for r in range(rows):
            ...
            if cid == 0:
                ...
                L = ((30 * (r / 3)) + (ecl * 3) + ((rows - 1) % 3))
            elif cid == 2:
                L = ((30 * (r / 3)) + (cols - 1))

            row += '{0:017b}'.format(self.CLUSTERS[cid][int(L)])

            for c in range(int(cols)):
                row += '{0:017b}'.format(self.CLUSTERS[cid][int(code_words[k])])
                k += 1
            ...
            for h in range(self.ROWHEIGHT):
                self.barcode_array['bcode'].append(arow)
        ...
        if self.QUIETV > 0:
            for i in range(self.QUIETV):
                self.barcode_array['bcode'].append(empty_row)

    def _get_sequence(self, code):
        seq_array = []

        numseq = re.findall('[0-9]{13,}', code)

        for n in range(0, len(numseq)):
            offset = code.index(numseq[n])
            numseq[n] = [numseq[n], offset]

        numseq.append(['', len(code)])
        ...
        for i in range(0, len(numseq)):
            ...
            if seq[1] > 0:
                ...
                for n in range(0, len(textseq)):
                    offset = prevseq.index(textseq[n])
                    textseq[n] = [textseq[n], offset]

                textseq.append(['', len(prevseq)])
                txtoffset = 0

                for j in range(len(textseq)):
                    txtseq = textseq[j]
                    txtseqlen = len(txtseq[0])
                    if txtseq[1] > 0:
                        prevtxtseq = prevseq[txtoffset:txtseq[1]-txtoffset]

                        if len(prevtxtseq) > 0:
                            if len(prevtxtseq) == 1 and len(seq_array) > 0 and seq_array[len(seq_array)-1][0] == 900:
                                seq_array.append([913, prevtxtseq])
                            elif (len(prevtxtseq) % 6) == 0:
                                seq_array.append([924, prevtxtseq])
                            else:
                                seq_array.append([901, prevtxtseq])

                    if txtseqlen > 0:
                        seq_array.append([900, txtseq[0]])

                    txtoffset = txtseq[1] + txtseqlen

            if seqlen > 0:
                seq_array.append([902, seq[0]])

            offset = seq[1] + seqlen

        return seq_array

    def _get_error_correction_level(self, ecl, numcw):
        maxecl = 8
        max_err_size = 928 - numcw
    ...
    def _get_error_correction(self, cw, ecl):
        ecc = self.RSFACTORS[ecl]

        ecl_size = 2 << ecl
        ecl_max_id = (ecl_size - 1)
        ecw = [0] * ecl_size

        for k in range(len(cw)):
            t1 = cw[k] + ecw[ecl_max_id] % 929
        ...
        for k, j in enumerate(range(len(ecw))):
            if ecw[j] != 0:
                ecw[j] = 929 - ecw[j]
    ...
    def _array_search(self, chval, haystack):
        try:
            return haystack.index(chval)
        except ValueError:
            return

    def _get_compaction(self, mode, code, add_mode=True):
        cw = []
        if mode == 900:
            ...
            for i in range(codelen):
                chval = ord(code[i])
                k = self._array_search(chval, self.TEXTSUBMODES[submode])

                if k is not None:
                    txtarr.append(k)
                else:
                    for s in range(4):
                        k = self._array_search(chval, self.TEXTSUBMODES[s])

                        if s != submode and k is not None:
                            if (((i + 1) == codelen) or (((i + 1) < codelen) and (self._array_search(ord(code[i + 1]), self.TEXTSUBMODES[submode]) is not None))) and ((s == 3) or ((s == 0) and (submode == 1))):
                                if s == 3:
                                    txtarr.append(29)
                                else:
                                    txtarr.append(27)
                            else:
                                txtarr = txtarr + self.TEXTLATCH['%s%s' % (submode, s)]
                                submode = s
            ...
            for i in range(0, txtarrlen, 2):
                cw.append(30 * int(txtarr[i]) + int(txtarr[i+1]))

        elif mode == 901 or mode == 924:
            # TODO
            pass
        elif mode == 902:
            # TODO
